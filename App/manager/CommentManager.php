...
class CommentManager extends Manager
{
    ...
    {
        $bd = $this->connection();
        $bdcommentsvalid = $bd->prepare('SELECT id, author, comment FROM comments WHERE post_id = ? AND valid = 1 ORDER BY id DESC');
    ...
    {
        $bd = $this->connection();
        $bdcomment = $bd->prepare('SELECT id, author, comment FROM comments WHERE id = ? AND valid = 1');
    ...
    public function showCommentsNotValid($postId){
        $bd = $this->connection();
        $bdcomments = $bd->prepare('SELECT id, author, comment FROM comments WHERE post_id = ? AND valid IS NULL ORDER BY id DESC');
    ...
    {
        $bd = $this->connection();
        $bdcommentadd = $bd->prepare('INSERT INTO comments(post_id, author, comment) VALUES(?, ?, ?)');
